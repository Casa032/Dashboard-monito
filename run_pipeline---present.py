...
def main():
    ...
    if not args.skip_ingest:
        ...
        try:
            ...
            if arch_p.exists():
                sm.sauvegarder_archivage(pd.read_parquet(arch_p))
                nb_arch = len(pd.read_parquet(arch_p))
                print(f"  ✓ ARCHIVAGE chargé ({nb_arch} projets archivés)")

            # Entités détectées
            entites = sm.lister_entites()
            if entites:
                print(f"  ✓ Entités détectées : {', '.join(entites)}")
