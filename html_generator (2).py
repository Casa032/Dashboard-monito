...
function switchQuinzaine(q){
  ...
  const gp=document.getElementById("page-gantt");if(gp&&gp.innerHTML.trim()!=="")buildGantt();
}
...
function buildGantt(){
  ...
  let html='<div class="gantt-wrap"><table class="gantt-table"><thead><tr><th class="g-label" style="font-family:var(--font-mono);color:var(--text3);font-size:9px">PROJET</th>';
  months.forEach(m=>{const k=m.toISOString().slice(0,7);html+='<th class="g-header'+(k===todayM?" g-today-head":"")+'">'+MFR[m.getMonth()]+'<br>'+String(m.getFullYear()).slice(2)+'</th>';});
  html+='</tr></thead><tbody>';
    ...
    items.forEach(p=>{
      ...
      if(mode==="dates"&&(m.date_debut||m.date_fin_prevue)){
        ...
        months.forEach(month=>{
          ...
          const bar=inRange?'<div style="position:absolute;top:8px;bottom:8px;left:2px;right:2px;border-radius:2px;background:'+col+';opacity:.65;border:1px solid rgba(255,255,255,.1)"></div>':"";
        ...
        months.forEach((month,mi)=>{
          ...
          const mEnd=new Date(month.getFullYear(),month.getMonth()+1,0);
          const nl=isT?'<div class="g-now" style="left:'+Math.round((now.getDate()-1)/mEnd.getDate()*100)+'%"></div>':"";
          const bar=op>0.04?'<div class="g-bar" style="left:2px;right:2px;background:'+col+';opacity:'+op.toFixed(2)+'"></div>':"";
          html+='<td class="g-cell">'+bar+nl+'</td>';
        });
      }
      html+='</tr>';
  ...
  let legend='<span><i style="width:12px;height:1.5px;background:var(--red);display:inline-block"></i> aujourd\\'hui</span>';
  Object.keys(groups).slice(0,8).forEach((g,i)=>{legend+='<span><i style="width:12px;height:5px;background:'+PALETTE[i%PALETTE.length]+';border-radius:2px;display:inline-block"></i>'+esc(g)+'</span>';});
  document.getElementById("gl").innerHTML=legend;
}
...
function renderEvolutions(){
  ...
  else html+='<div>'+delta.map(d=>{const dv=d.delta_avancement||0;const sign=dv>0?"+":"";const dc=dv>0?"var(--green)":dv<0?"var(--red)":"var(--text3)";const stChange=d.statut_avant&&d.statut_apres&&d.statut_avant!==d.statut_apres?'<span style="font-size:9px;color:var(--text3);font-family:var(--font-mono)">'+d.statut_avant+' -> '+d.statut_apres+'</span>':"";const dvTag=dv!==0?'<span style="font-size:9px;font-family:var(--font-mono);color:'+dc+'">'+sign+Math.round(dv)+'%</span>':"";return'<div class="tl-item"><div class="tl-dot" style="background:'+dc+'"></div><div class="tl-body"><div class="tl-title" onclick="openModal(\''+esc(d.projet_id||d.ref_sujet)+'\')">'+esc(d.projet_nom||d.sujet||d.projet_id)+'</div><div class="tl-meta">'+stChange+dvTag+badge(d.statut_apres||"ON_HOLD")+'</div></div></div>';}).join('')+'</div>';
  html+='</div>';
...
def generer_html(donnees, llm_cache=None):
    ...
    html += '<div class="logo">'
    html += '<div class="logo-title">DATA_MONITOR</div>'
    html += '<div class="logo-sub">// pilotage projets</div>'
    ...
    html += '<div class="nav-item" data-page="chat"><span class="nav-icon">&#8984;</span>Chat LLM</div>'
    ...
    html += '<button class="btn-pdf" onclick="window.print()">EXPORT PDF</button>'
    html += '</div>'
