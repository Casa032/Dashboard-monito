...
Nouveautés :
    - charger_archivage() : projets terminés depuis archivage.parquet
    - lister_entites()    : entités uniques extraites de entite_concerne (multi-valeurs)
    - charger_agenda()    : événements depuis agenda.parquet
"""
...
class StorageManager:
    ...
    def charger_archivage(self, mois_glissants: int = 12) -> pd.DataFrame:
        ...
        if mois_glissants > 0 and "date_fin_prevue" in df.columns:
            cutoff = (datetime.now() - timedelta(days=mois_glissants * 30)).strftime("%Y-%m-%d")

            def _parse_date_fin(v):
            ...
            df["_date_fin_norm"] = df["date_fin_prevue"].apply(_parse_date_fin)
            # Garder les projets sans date (pour ne pas les perdre) + ceux dans la fenêtre
            df = df[(df["_date_fin_norm"].isna()) | (df["_date_fin_norm"] >= cutoff)]
            df = df.drop(columns=["_date_fin_norm"])
