...
TYPES_AGENDA_VALIDES = {"REUNION", "LIVRAISON", "ACTUALITE", "JALON", "EVENEMENT", "AUTRE"}
...
def parser_fiches(
    ...
    # ── AGENDA consolidé ──────────────────────────────────────────────
    # ── Extraire faits_marquants de META et les fusionner dans agenda ──
    faits_entries = []
    if not df_meta_global.empty and "faits_marquants" in df_meta_global.columns:
        col_id_m  = "projet_id" if "projet_id" in df_meta_global.columns else "ref_sujet"
        col_nom_m = "projet_nom" if "projet_nom" in df_meta_global.columns else "sujet"
        for _, row in df_meta_global.iterrows():
            raw = str(row.get("faits_marquants", "") or "").strip()
            if not raw or raw in ("nan", "None", ""):
                continue
            nom_proj = str(row.get(col_nom_m, "") or "")
            for paire in raw.split(";"):
                ...
                try:
                    ...
                    log.warning(f"faits_marquants date non parsée : {date_str}")
                    continue
                faits_entries.append({
                    "date":        date_norm,
                    "titre":       f"Événement — {nom_proj}" if nom_proj else "Événement projet",
                    "type":        "EVENEMENT",
                    "description": desc,
                    "projet_ref":  str(row.get(col_id_m, "") or ""),
                    "source_fichier": "faits_marquants_meta",
                })
        if faits_entries:
            log.info(f"faits_marquants → {len(faits_entries)} événement(s) extraits de META")
    ...
    if agendas or faits_entries:
        ...
        # S'assurer que toutes les colonnes AGENDA sont présentes
    ...
    elif not agendas and not faits_entries:
        pass  # pas d agenda
