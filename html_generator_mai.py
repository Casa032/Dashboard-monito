...
function projItem(p){
  ...
  const prioTag=prio&&prio!=="nan"?`<span style="font-size:9px;font-family:var(--font-mono);color:var(--text3);white-space:nowrap">P:${esc(prio)}</span>`:"";
...
function filterOverviewByEntite(ent){
  ...
  const top=[...DATA.projets].filter(p=>_projetMatchEntite(p,ent)).sort((a,b)=>
    ({"En retard":0,"À risque":1,"En cours":2,"Stand by":3,"Terminé":4}[a.statut]||9)-
...
function renderOverview(){
  ...
  const top=[...P].sort((a,b)=>({"En retard":0,"À risque":1,"En cours":2,"Stand by":3,"Terminé":4}[a.statut]||9)-({"En retard":0,"À risque":1,"En cours":2,"Stand by":3,"Terminé":4}[b.statut]||9)).slice(0,10);
...
function renderGantt(){
  ...
  el.innerHTML = `
    <div class="gantt-toolbar">
      ...
      <span style="font-size:9px;color:var(--text3);font-family:var(--font-mono);margin-left:4px">domaine :</span>
      <select id="gdom" onchange="ganttFiltreDomaine=this.value;buildGantt()">
        <option value="">Tous</option>
        ${(DATA.domaines||[]).map(d=>`<option value="${esc(d)}">${esc(d)}</option>`).join("")}
      </select>
      <span style="font-size:9px;color:var(--text3);font-family:var(--font-mono);margin-left:4px">collab :</span>
      <select id="gcol" onchange="ganttFiltreCollab=this.value;buildGantt()">
        <option value="">Tous</option>
        ${Object.keys(DATA.par_resp||{}).sort().map(r=>`<option value="${esc(r)}">${esc(r)}</option>`).join("")}
      </select>
    </div>
  ...
  return`
    ...
    <div class="grid2">
      ...
      <div class="card">
        ...
        ${barChart(
          Object.entries({"En cours":last.projets?.filter(p=>p.statut==="En cours").length||0,
            "À risque":last.projets?.filter(p=>p.statut==="À risque").length||0,
            "En retard":last.projets?.filter(p=>p.statut==="En retard").length||0,
            "Terminé":last.projets?.filter(p=>p.statut==="Terminé").length||0,
            "Stand by":last.projets?.filter(p=>p.statut==="Stand by").length||0})
